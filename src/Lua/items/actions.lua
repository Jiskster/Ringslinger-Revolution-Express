...
-- P_DustRing ported from C to Lua, since it's not exposed, for whatever reason
-- TODO: Remove this when said function gets exposed to Lua
local RSR_DustRing = function(mobjtype, div, x, y, z, radius, speed, initscale, scale)
...
-- TODO: Consider doing that for a standalone release
A_TNTExplode = function(actor, var1, var2)
	...
	if var1 then
		RSR_DustRing(var1, 4, actor.x, actor.y, actor.z + actor.height, 64, 0, FRACUNIT, actor.scale)
		RSR_DustRing(var1, 6, actor.x, actor.y, actor.z + actor.height/2, 96, FRACUNIT, FRACUNIT, actor.scale)
	end
...
-- TODO: Make sure this lines up with the actual NUMPOWERS after 2.2.13 and 2.2.14
...
-- 		print(string.format("sending player %s's items to %s", send_pl, recv_pl))
		...
		if players[recv_pl].rsrinfo then
			...
			RSR.DrawWeapon(players[recv_pl], weaponheld[send_pl+1])
